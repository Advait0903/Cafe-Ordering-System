...
public class CoffeeShop {
    ...
    private final ArrayList<Coffee> cart = new ArrayList();
    ...
    public void addNewIndianCoffee(String id, String name, String description, String size, double price) {
        menu.add(new IndianCoffee(id, name, description, size, price));
    }

    public void printMenuByChoosing() {
        //Ask user how they want to view
        System.out.println("1. View menu by default");
        System.out.println("2. View menu ascending by Coffee name");
        System.out.println("3. View menu ascending by Coffee price");
    ...
    private void printMenuAscendingByID() {
        //Sorting Coffee ID by using Comparable
        //By implementing Comparable interface in Coffee class and @Override compareTo() method
        Collections.sort(menu);
        printMenu();
    }

    private void printMenuAscendingByName() {
        //Sorting Coffee name by using Comparator, Anonymous class
        Comparator<Coffee> nameBalance = new Comparator<Coffee>() {
            @Override
    ...
    private Coffee getCoffeeFromMenuByID (String id) {
        for (Coffee x : menu)
    ...
    private String getIDFromUser() {
        String inputMsg = "Input Coffee ID: ";
    ...
    public void printCart() {
        ...
        System.out.println(header);
        //if cart has Coffee then print it all
        if (!cart.isEmpty())
            for (Coffee x : cart) {
                String outputMsg;
                outputMsg = String.format("|%-20s|%5s|%-20s|%-40s|%2s%2s|%3d|%,13.0f|\n", x.getCoffeeType(), x.getId(),
    ...
    public void addToCartByID() {
        ...
        Coffee CoffeeByID = getCoffeeFromMenuByID(id);
        if (CoffeeByID == null)
    ...
    public void makePayment() {
        System.out.printf("Your total payment is %,.0f Rs.\n", totalPayment());
